...
def _draw_kpi_cards(ax, items: List[dict]):
    ...
    for idx, it in enumerate(items[: rows*cols ]):
        ...
        patch = FancyBboxPatch(
            ...
            facecolor=CARD_BG, edgecolor=CARD_EDGE, linewidth=1.0,
        ...
        if dt:
            ax.text(
                x0 + card_w - 0.03, y0 + card_h - 0.08,
                dt,
                fontsize=12, color=(dc or TEXT_MUTED),
...
async def build_report_pdf_bytes(
    ...
    *,
    ...
    avg_dau      = round(sum(int(c) for _, c in dau_curr) / max(len(dau_curr), 1), 1) if dau_curr else 0.0
    ...
    prev_avgdau  = round(sum(int(c) for _, c in dau_prev) / max(len(dau_prev), 1), 1) if dau_prev else 0.0
    ...
    with PdfPages(pdf_buf) as pdf:
        # 1) Cover
        fig = _new_fig(); ax = fig.add_subplot(); ax.axis("off")
        ax.text(0.05, 0.80, t("reports.cover.title", user_id=user_id, lang=lang), fontsize=28, fontweight="bold", transform=ax.transAxes)
        ax.text(0.05, 0.70, t("reports.cover.subtitle", user_id=user_id, lang=lang, start=start_str, end=end_str), fontsize=14, transform=ax.transAxes)
        ax.text(0.05, 0.62, t("reports.cover.chat", user_id=user_id, lang=lang, title=(chat_title or str(chat_id))), fontsize=13, transform=ax.transAxes)
        ax.text(0.05, 0.54, t("reports.cover.generated", user_id=user_id, lang=lang, dt=now_utc.strftime("%Y-%m-%d %H:%M")), fontsize=11, color="#555", transform=ax.transAxes)
        fig.tight_layout(); pdf.savefig(fig); plt.close(fig)

        # 2) KPI cards
        fig = _new_fig(); ax = fig.add_subplot()
        ax.text(0.05, 0.92, t("reports.kpi.title", user_id=user_id, lang=lang), fontsize=18, fontweight="bold", transform=ax.transAxes)
        items = [
            dict(
                label=t("reports.kpi.joins", user_id=user_id, lang=lang),
                value=f"{total_joins:,}",
                sublabel=t("reports.kpi.window", user_id=user_id, lang=lang, n=window_days),
                delta_text=d_joins_txt, delta_color=d_joins_col,
            ),
            dict(
                label=t("reports.kpi.leaves", user_id=user_id, lang=lang),
                value=f"{total_leaves:,}",
                sublabel=t("reports.kpi.window", user_id=user_id, lang=lang, n=window_days),
                delta_text=d_leaves_txt, delta_color=d_leaves_col,
            ),
            dict(
                label=t("reports.kpi.net", user_id=user_id, lang=lang),
                value=f"{net_growth:,}",
                sublabel=t("reports.kpi.window", user_id=user_id, lang=lang, n=window_days),
                delta_text=d_net_txt, delta_color=d_net_col,
            ),
            dict(
                label=t("reports.kpi.messages", user_id=user_id, lang=lang),
                value=f"{total_msgs:,}",
                sublabel=t("reports.kpi.window30", user_id=user_id, lang=lang),
                delta_text=d_msgs_txt, delta_color=d_msgs_col,
            ),
            dict(
                label=t("reports.kpi.avg_dau", user_id=user_id, lang=lang),
                value=f"{avg_dau:,}",
                sublabel=t("reports.kpi.window30_users", user_id=user_id, lang=lang),
                delta_text=d_dau_txt, delta_color=d_dau_col,
            ),
            dict(
                label=t("reports.kpi.peak_hour", user_id=user_id, lang=lang),
                value=(f"{peak_hour}:00" if peak_info else "—"),
                sublabel=t("reports.kpi.peak_count", user_id=user_id, lang=lang, n=peak_count) if peak_info else "",
        ...
        if top_user_id is not None:
            ax.text(
                0.05, 0.06,
                t("reports.kpi.top_user", user_id=user_id, lang=lang, top_user_id=top_user_id, count=top_user_cnt),
        ...
        if jl_curr:
            ...
            ax.plot(x, joins_y, linewidth=2.2, label=t("reports.kpi.joins", user_id=user_id, lang=lang))
            ax.plot(x, leaves_y, linewidth=2.2, label=t("reports.kpi.leaves", user_id=user_id, lang=lang))
        ...
        if top5:
            fig = _new_fig(); ax = fig.add_subplot()
            ax.set_title(t("reports.top_talkers.title", user_id=user_id, lang=lang), fontsize=14, fontweight="bold")
            ax.axis("off")
            headers = [
            ...
            y_text = 0.85
            ax.text(0.06, y_text, f"{headers[0]:<18}  {headers[1]:>10}", family="monospace", fontsize=12, transform=ax.transAxes)
            y_text -= 0.03
            ax.text(0.06, y_text, "-"*34, family="monospace", color="#666", transform=ax.transAxes)
            for uid, total in top5:
                y_text -= 0.05
                ax.text(0.06, y_text, f"{str(uid):<18}  {total:>10}", family="monospace", fontsize=12, transform=ax.transAxes)
            fig.tight_layout(); pdf.savefig(fig); plt.close(fig)
